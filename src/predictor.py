...
# Define the Predictor class which wraps a model for easier use.
class Predictor(nn.Module):

    def __init__(self, model, class_names, mean, std):
        super().__init__()

        # Store the model, class names and normalization parameters.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            # Apply transformations, pass the tensor through the model and apply softmax.
            x  = self.transforms(x)
            x  = self.model(x)
            x  = F.softmax(x, dim=1)
...
# Define a function to test the predictor.
